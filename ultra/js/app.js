...
const LS = {
  ...
  apiBankCache: (id) => 'ultra_apiBank_' + id, // API 题库题目缓存
...
let state = {
  ...
  fav: {},
  editedAnswers: {},
  user: null,        // 当前登录用户 {id, email, nickname, role}
};
...
async function loadBank(bankId) {
  ...
  if (bankId === 'default') {
    ...
    // 1. 先查 localStorage 本地题库
    const banks = getUserBanks();
    bank = banks.find(b => b.id === bankId);
    // 2. 再查 API 题库（数字 ID，来自服务端）
    if (!bank && state.user) {
      const apiBankMeta = apiBanks.find(b => String(b.id) === String(bankId));
      if (apiBankMeta) {
        // 尝试从缓存加载
        const cached = localStorage.getItem(LS.apiBankCache(bankId));
        if (cached) {
          try { bank = JSON.parse(cached); } catch(e) {}
        }
        // 尝试从 API 加载
        if (!bank || !bank.questions) {
          try {
            const qRes = await API.banks.getQuestions(bankId);
            bank = {
              id: String(apiBankMeta.id),
              name: apiBankMeta.name,
              questions: qRes.questions || [],
              _api: true,
              _is_public: apiBankMeta.is_public,
              _owner_id: apiBankMeta.owner_id,
            };
            localStorage.setItem(LS.apiBankCache(bankId), JSON.stringify(bank));
          } catch (e) {
            console.warn('API 题库加载失败:', e.message);
            if (!bank) {
              alert('题库加载失败：' + bankId);
              return;
            }
...
function renderBankSelect() {
  ...
  // 合并 API 题库（服务端的，未在本地导入的）
  if (state.user && apiBanks.length > 0) {
    const localIds = new Set(banks.map(b => b.id));
    for (const ab of apiBanks) {
      if (localIds.has(String(ab.id))) continue; // 已在本地，跳过
      const cached = localStorage.getItem(LS.apiBankCache(ab.id));
      let count = 0;
      if (cached) { try { count = JSON.parse(cached).questions.length || 0; } catch(e) {} }
      html += renderBankItem(String(ab.id), ab.name, count, activeId === String(ab.id), false);
    }
  }
...
// 从 API 加载题库列表（登录后调用）
async function loadApiBanks() {
  if (!state.user) { apiBanks = []; return; }
  try {
    const res = await API.banks.list();
    apiBanks = (res.banks || []).filter(b => !b.is_default);
  } catch (e) {
    console.warn('加载 API 题库列表失败:', e.message);
    apiBanks = [];
  }
}
...
function saveState() {
  ...
  // API 同步（fire-and-forget）：仅对 API 题库同步题目内容
  if (state.user && state.bank._api) {
    API.banks.putQuestions(state.bank.id, state.questions).catch(e => {
      console.warn('API 题库同步失败（不影响本地使用）:', e.message);
    });
  }
}
